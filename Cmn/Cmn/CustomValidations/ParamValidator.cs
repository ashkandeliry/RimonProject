...
namespace Common.CustomValidations
{
    ...
    {
        public static string ValidateNationCode(string nationalCode, bool allowNull)
        {
            if (allowNull)
            {
            ...
            {
                if (string.IsNullOrEmpty(nationalCode) || string.IsNullOrWhiteSpace(nationalCode))
                    throw new BusinessException(XError.InputErrors_.NationalCodeFieldIsNull());
            }

            nationalCode = nationalCode.RemoveWhiteSpaces();
            nationalCode = nationalCode.Replace("-", "");

            if (nationalCode.Length != 10)
                throw new BusinessException(XError.InputErrors_.NationalCodeNotValid(nationalCode));
